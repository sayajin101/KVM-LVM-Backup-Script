...
for backupList in `grep -v '^#' ${configFile} | awk '{print $1}'`; do

	iName=$(grep "${backupList}" ${configFile} | awk '{print $1}');
	iVolumeGroup=$(grep "${backupList}" ${configFile} | awk '{print $2}');
...
#	virsh dumpxml mail10.megs.co.za > meh.xml
	if [ "${iCompressionLocation}" == "remote" ]; then
		...
		copyBackup() {
			...
			[ "${copyCount}" -gt "3" ] && { log error "Error: Copy failed after 3 attempts for ${iVolumeGroup}-${iName} to backup server ${remoteAddress}." && continue; };
			/bin/dd if=${lv_path}_snap bs=16MB | ssh -i ${scriptPath}/key/lvm-backup -p ${remotePort} ${remoteUser}@${remoteAddress} "/bin/gzip -c | /bin/dd of=${iRemotePath}/vms/${iVolumeGroup}-${iName}.${date}.gz;";
			gzipIntegrity=$(ssh -i ${scriptPath}/key/lvm-backup -p ${remotePort} ${remoteUser}@${remoteAddress} "gunzip -t ${iRemotePath}/vms/${iVolumeGroup}-${iName}.${date}.gz > /dev/null 2>&1; echo $?;";);
			if [ "${gzipIntegrity}" -ne "0" ]; then
				...
				${lvr} -f ${lv_path}_snap;
				log success "Copy for ${iVolumeGroup}-${iName} to backup server ${remoteAddress} was successful.";
			fi;
		};
		copyBackup;
		...
		copyBackup() {
			[ -z "${copyCount}" ] && copyCount="1";
			[ "${copyCount}" -gt "3" ] && { log error "Error: Copy failed after 3 attempts for ${iVolumeGroup}-${iName} to backup server ${remoteAddress}." && continue; };
			/bin/dd if=${lv_path}_snap bs=16MB | /bin/gzip -c | ssh -i ${scriptPath}/key/lvm-backup -p ${remotePort} ${remoteUser}@${remoteAddress} "/bin/dd of=${iRemotePath}/vms/${iVolumeGroup}-${iName}.${date}.gz;";
			gzipIntegrity=$(ssh -i ${scriptPath}/key/lvm-backup -p ${remotePort} ${remoteUser}@${remoteAddress} "gunzip -t ${iRemotePath}/vms/${iVolumeGroup}-${iName}.${date}.gz > /dev/null 2>&1; echo $?;";);
			if [ "${gzipIntegrity}" -ne "0" ]; then
				log error "Backup file integrity error...backup file restarting backup procedure";
				(( copyCount++ ));
